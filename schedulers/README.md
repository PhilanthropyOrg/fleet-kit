...
| hq-prod-alert | event-driven (fires on write) | `scripts/notify_hq_prod_alert.py` | optional — hands every new prod alert START/RESOLVE (`webhook_receiver.py`'s `/webhook/prod-alert` → `logs/prod-alerts.jsonl`) to Reif HQ as one headless job via `~/reif-chat/job.sh` (never tmux send-keys). `fleetkit-hq-prod-alert.path` watches the file, `.service` does the hand-off; systemd-only |
