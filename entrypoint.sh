...
case "${1:-cron-foreground}" in
  ...
  cron-foreground)
    ...
    CRONTAB=/etc/cron.d/fleet-kit
    {
      ...
      # for FLEET_REPO_URL / FLEET_RUN_PLAIN; FLEET_VIEW_PORT is this container's own.
      echo "38 * * * * root export GH_TOKEN=\$(cat $TOKEN_FILE) FLEET_LOG_DIR=$LOG_DIR FLEET_VIEW_PORT=\${FLEET_VIEW_PORT:-8420} && [ -f \"\${FLEET_ENV_FILE:-/fleet-kit/fleet.env}\" ] && { set -a; . \"\${FLEET_ENV_FILE:-/fleet-kit/fleet.env}\"; set +a; }; python3 /fleet-kit/scripts/reif_eyes.py >> $LOG_DIR/reif_eyes.log 2>&1"
